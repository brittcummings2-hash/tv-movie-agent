...
export async function POST(request: Request) {
  try {
    ...
    const { posters, overviews, platforms, releaseDates, episodeCounts, nextEpisodes, seriesStatuses } =
      await resolveLibraryMedia(items);
    return NextResponse.json(
      { posters, overviews, platforms, releaseDates, episodeCounts, nextEpisodes, seriesStatuses },
